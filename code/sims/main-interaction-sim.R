# main-interaction-sim.R: script to perform the simulation study comparing the
#                         five candidate models in their ability to predict
#                         a species-environment relationship from the "Interaction"
#                         scenario.
...
for (j in 1:n.sims) {
  ...
  psi <- logit.inv(beta.0 + beta.linear * x.1 + beta.interaction * x.1 * x.interaction)
  z <- rbinom(J, 1, psi)
  ...
  data.list <- list(y = y, 
		    occ.covs = data.frame(x = x.1, 
					  ...
					  strata = strata), 
		    coords = coords)
  beta.effect.true.vals[j, ] <- beta.linear + beta.interaction * x.interaction
...
save(waic.vals, beta.effect.true.vals, beta.effect.mean.vals, 
     file = 'results/sim-true-interaction.rda')
