# main-bcr-stPGOcc.R: this script contains code to run a spatial multi-season
#                     occupancy model for each of the 51 forest bird species
#                     in which we fit a model with a trend that varies
#                     across bird conservation regions.
...
out <- stPGOcc(occ.formula = ~ scale(years):factor(BCR) + scale(tmax),
...
# Save result -------------------------------------------------------------
save(out, file = paste("results/case-study-1/stPGOcc-bcr-", curr.sp, ".rda", sep = ''))
