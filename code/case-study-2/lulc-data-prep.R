# lulc-data-prep.R: this script extracts the LULC data at all BBS routes
#                   in the data set for generation of the grassland and 
#                   cropland covariates. Data are extracted from
#                   USGS EROS. 
...
# Reset CRS in case using a different version of GDAL.
st_crs(routes.final) <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"

# Convert routes to Albers Equal Area
coords.albers <- routes.final %>%
...
save(grass.cov, crop.cov, file = paste("data/case-study-2/yearly-lulc-500/lulc-vals-", curr.year, sep = ''))
