...
#                   the trend to vary smoothly across space.
...
out <- svcTPGOcc(occ.formula = ~ scale(years) + scale(tmax), 
...
# Save result -------------------------------------------------------------
save(out, file = paste("results/case-study-1/svcTPGOcc-", curr.sp, ".rda", sep = ''))
