# summary-sim-2.R: this script summarizes the second simulation study from
...
# Load results ------------------------------------------------------------
load("results/sim-2-results.rda")
...
plot.psi.abs.bias %>%
  group_by(p.svc) %>%
  summarize(avg.bias = mean(psi.abs.bias))
# Grouped by SVC and Number of sites. This is part of Table S2.
plot.psi.abs.bias %>%
  group_by(p.svc, J) %>%
  summarize(avg.bias = mean(psi.abs.bias)) %>%
  print(n = nrow(.))
...
plot.w.abs.bias %>%
  group_by(p.svc) %>%
  summarize(avg.bias = mean(w.abs.bias))
# Grouped by SVC and Number of sites. This is part of Table S3
plot.w.abs.bias %>%
  group_by(p.svc, J) %>%
  summarize(avg.bias = mean(w.abs.bias)) %>%
  print(n = nrow(.))
...
ggarrange(psi.bias.plot, psi.ci.width.plot, cor.w.plot, nrow = 1, common.legend = TRUE, 
          legend = 'bottom')
ggsave(file = 'figures/Fig-1.pdf', units = 'in', device = 'pdf', width = 15, height = 6)
