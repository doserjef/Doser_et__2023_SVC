# main-linear-sim.R: script to perform the simulation study comparing the 
#                    five candidate models in their ability to predict
#                    a species-environment relationship from the "Linear" 
#                    scenario.
...
for (j in 1:n.sims) {
  ...
  psi <- logit.inv(beta.0 + beta.linear * x.1)
  z <- rbinom(J, 1, psi)
  ...
  data.list <- list(y = y, 
		    occ.covs = data.frame(x = x.1, 
					  ...
					  strata = strata), 
		    coords = coords)
  beta.effect.true.vals[j, ] <- beta.linear
  ...
  for (i in 1:n.models) {
    ...
    if (curr.model != 'svc') {
      ...
      # Run the SVC model for a bit longer, since it takes longer to converge
      n.samples <- 30000
      batch.length <- 25
      n.batch <- n.samples / batch.length
      n.burn <- 20000
...
save(waic.vals, beta.effect.true.vals, beta.effect.mean.vals, 
     file = 'results/sim-true-linear.rda')
