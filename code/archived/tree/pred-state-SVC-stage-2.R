...
colnames(X.0) <- c('intercept', 'vpd', 'ppt', 'elev', 'elev.2')
X.0[, 'vpd'] <- (covs.0[, 'vpd']) / sd(data.list.2$covs[, 'vpd'])
X.0[, 'ppt'] <- (covs.0[, 'ppt']) / sd(data.list.2$covs[, 'ppt'])
...
# TODO: temporary
# y.0.means <- apply(out.pred$y.0.samples, 2, mean)
