# main-strata-sim.R: script to perform the simulation study comparing the 
#                  five candidate models in their ability to predict
#                  a species-environment relationship from the "Stratum" 
#                  scenario.
...
for (j in 1:n.sims) {
  ...
  psi <- logit.inv(beta.0 + beta.linear * x.1 + beta.strata[strata] * x.1)
  z <- rbinom(J, 1, psi)
  ...
  data.list <- list(y = y, 
		    occ.covs = data.frame(x = x.1, 
					  ...
					  strata = strata), 
		    coords = coords)
  beta.effect.true.vals[j, ] <- beta.linear + beta.strata[strata]
...
save(waic.vals, beta.effect.true.vals, beta.effect.mean.vals, 
     file = 'results/sim-true-strata.rda')
