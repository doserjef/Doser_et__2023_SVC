# pred-full.R: script to predict occupancy probability of Grasshopper 
#              Sparrow across the US from 1970-2019, and extract the SVC
#              effects for subsequent mapping.
...
psi.mean.samples <- array(NA, dim = c(n.samples, J.0))
for (j in 1:length(vals)) {
  print(paste("Currently on set ", j, " out of ", length(vals), sep = ''))
  curr.indx <- vals[[j]]
  out.pred <- predict(out, X.0[curr.indx, , ], coords.0[curr.indx, ],
        	      t.cols = 1:n.time, n.omp.threads = 10, verbose = FALSE)
  w.samples[, , curr.indx] <- out.pred$w.0.samples
  psi.mean.samples[, curr.indx] <- apply(out.pred$psi.0.samples, c(1, 2), mean)
}
save(w.samples, file = 'results/case-study-2/full-pred-results.rda')
save(psi.mean.samples, file = 'results/case-study-2/full-psi-pred-results.rda')
