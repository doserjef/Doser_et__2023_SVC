# main-stPGOcc.R: this script contains code to run the "BCR" model, which 
#                 is a spatial occupancy model with a separate year trend
#                 for each BCR. 
...
# Fill in BCRs with missing values
data.list$occ.covs$BCR[559] <- 29
data.list$occ.covs$BCR[836] <- 14
data.list$occ.covs$BCR[929] <- 30
data.list$occ.covs$BCR[931] <- 14
data.list$occ.covs$BCR[1332] <- 26
data.list$occ.covs$BCR[1820] <- 27
...
out <- stPGOcc(occ.formula = ~ scale(years):factor(BCR), 
...
# Save result -------------------------------------------------------------
save(out, file = paste("results/stPGOcc-bcr-", curr.sp, ".R", sep = ''))
