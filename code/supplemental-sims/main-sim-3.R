# main-sim-3.R: code to perform the third supplemental simulation study to assess how 
#               sample size and the number of time periods influences 
#               the bias and precision of an SVC trend from a multiseason
#               SVC occupancy model.
...
w.true <- array(NA, dim = c(max(J.vals), 2, n.sims, n.scenarios))
...
w.mean.samples <- array(NA, dim = c(max(J.vals), 2, n.sims, n.scenarios))
w.low.samples <- array(NA, dim = c(max(J.vals), 2, n.sims, n.scenarios))
w.high.samples <- array(NA, dim = c(max(J.vals), 2, n.sims, n.scenarios))
...
for (j in 1:n.sims) {
  ...
  for (i in 1:n.scenarios) {
    ...
    w.true[1:J.curr, 1:p.svc, j, i] <- dat$w
    ...
    # SVC model
    out <- svcTPGOcc(occ.formula = ~ trend, 
    ...
    w.mean.samples[1:J.curr, 1:p.svc, j, i] <- t(apply(out$w.samples, c(2, 3), mean))
    w.low.samples[1:J.curr, 1:p.svc, j, i] <- t(apply(out$w.samples, c(2, 3), 
						   quantile, 0.025))
    w.high.samples[1:J.curr, 1:p.svc, j, i] <- t(apply(out$w.samples, c(2, 3), 
						    quantile, 0.975))
  } # i (n.scenarios)
} # j (n.sims)
...
save(psi.mean.samples, psi.low.samples, psi.high.samples, 
     ...
     psi.true, w.true, scenario.vals, file = 'results/supplemental-sims/sim-3-results.rda')
