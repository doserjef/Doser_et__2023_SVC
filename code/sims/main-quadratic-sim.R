# main-quadratic-sim.R: script to perform the simulation study comparing the
#                       five candidate models in their ability to predict
#                       a species-environment relationship from the "Quadratic"
#                       scenario.
...
for (j in 1:n.sims) {
  ...
  psi <- logit.inv(beta.0 + beta.linear * x.1 + beta.quadratic * I(x.1^2))
  z <- rbinom(J, 1, psi)
  ...
  data.list <- list(y = y, 
		    occ.covs = data.frame(x = x.1, 
					  ...
					  strata = strata), 
		    coords = coords)
  beta.effect.true.vals[j, ] <- beta.linear + beta.quadratic * x.1 * 2
...
save(waic.vals, beta.effect.true.vals, beta.effect.mean.vals, 
     file = 'results/sim-true-quadratic.rda')
