# main-svcTPGOcc.R: this script contains code to a spatially-varying coefficients
#                   model for each of the 51 forest bird species, where we allow
#                   the trend to vary across space.
...
tuning.list <- list(phi = 0.1, rho = 0.5, sigma.sq = 0.3)
...
priors <- list(sigma.sq.unif = list(a = 0, b = 10), 
               phi.unif = list(a = 3 / max.dist, b = 3 / low.dist))
...
# Save result -------------------------------------------------------------
save(out, file = paste("results/svcTPGOcc-", curr.sp, ".R", sep = ''))
